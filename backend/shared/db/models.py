...
class Organization(Base, TimestampMixin):
    ...
    # Relationships
    creator: Mapped["User"] = relationship("User", back_populates="organizations_created", foreign_keys=[created_by])
...
class Project(Base, TimestampMixin):
    ...
    creator: Mapped["User"] = relationship("User", back_populates="projects_created", foreign_keys=[created_by])
...
class Document(Base, TimestampMixin):
    ...
    vector_id: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, unique=True)
    ...
    _old_content: ClassVar[Optional[str]] = None
    ...
    gaps: Mapped[List["Gap"]] = relationship("Gap", back_populates="document", cascade="all, delete-orphan")
    source_relationships: Mapped[List["DocumentRelationship"]] = relationship(
        "DocumentRelationship", foreign_keys="DocumentRelationship.source_document_id", back_populates="source_document"
    )
    target_relationships: Mapped[List["DocumentRelationship"]] = relationship(
        "DocumentRelationship", foreign_keys="DocumentRelationship.target_document_id", back_populates="target_document"
    )
...
class DocumentRelationship(Base, TimestampMixin):
    ...
    # Relationships
    source_document: Mapped["Document"] = relationship("Document", foreign_keys=[source_document_id], back_populates="source_relationships")
    target_document: Mapped["Document"] = relationship("Document", foreign_keys=[target_document_id], back_populates="target_relationships")
...
class Gap(Base, TimestampMixin):
    ...
    __table_args__ = (
        CheckConstraint("priority IN ('critical', 'high', 'medium', 'low')", name='ck_gaps_priority'),
        CheckConstraint("status IN ('pending', 'responded', 'rejected')", name='ck_gaps_status'),
    )
...
class Question(Base, TimestampMixin):
    ...
    __table_args__ = (
        CheckConstraint("status IN ('pending', 'incomplete', 'answered', 'verified')", name='ck_questions_status'),
    )
...
class Proposal(Base, TimestampMixin):
    ...
    __table_args__ = (
        CheckConstraint("status IN ('pending', 'accepted', 'rejected', 'implemented')", name='ck_proposals_status'),
    )
...
class ProposalDocument(Base, TimestampMixin):
    ...
    proposal_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey('proposals.id', ondelete='CASCADE'), primary_key=True
    )
    document_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey('documents.id', ondelete='CASCADE'), primary_key=True
    )
...
class VectorSyncLog(Base, TimestampMixin):
    ...
    __table_args__ = (
        CheckConstraint("entity_type IN ('document', 'question')", name='ck_sync_entity_type'),
        CheckConstraint("sync_status IN ('pending', 'synced', 'failed')", name='ck_sync_status'),
        CheckConstraint("sync_action IN ('create', 'update', 'delete')", name='ck_sync_action'),
    )
...
class QdrantCollection(Base, TimestampMixin):
    ...
    __table_args__ = (
        CheckConstraint("distance_metric IN ('cosine', 'euclidean', 'dot')", name='ck_qdrant_distance'),
    )
